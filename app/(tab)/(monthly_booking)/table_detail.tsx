...
type ListTableRouteProp = RouteProp<RootStackParamList, "table_detail_monthly">;

type Props = {
  route: ListTableRouteProp;
};

export default function MonthlyTableDetail({ route }: Props) {
  ...
  const handleToggleTable = async (table: ChessTable) => {
    try {
      ...
      Toast.show({
        type: "success",
        text1: isTableSelected(selectedTables, table)
          ? "Đã bỏ chọn bàn"
          : "Đã thêm bàn vào danh sách",
      });
    } catch (error) {
  ...
  const handleClearTables = async () => {
    ...
    Toast.show({
      type: "success",
      text1: "Đã xóa tất cả bàn đã chọn",
    });
  };
